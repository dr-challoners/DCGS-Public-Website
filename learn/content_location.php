...
elseif (isset($_GET['page'])) { // If this is false, then the INDEX page content will be returned instead (below)
	...
	$folders = scandir($contentpath.$getSubject, 1);
	foreach ($folders as $folder) {
		...
		if (strtolower($foldername) == strtolower($getFolder)) {
			$subfolders = scandir($contentpath.$getSubject."/".$folder, 1);
			break;
			}
		}
	if (isset($_GET['subfolder'])) {
		foreach ($subfolders as $subfolder) {
			...
			if (strtolower($subfoldername) == strtolower($getSubfolder)) {
				$pages = scandir($contentpath.$getSubject."/".$folder."/".$subfolder, 1);
				break;
				}
			}
		foreach ($pages as $page) {
			...
			if (strtolower($pagename) == strtolower($getPage)) {
				$parts = scandir($contentpath.$getSubject."/".$folder."/".$subfolder."/".$page, 1);
				$page_check = 1;
				break;
				}
			}
		...
		foreach ($subfolders as $page) {
			...
			if (isset($pagename[1])) { $pagename = $pagename[1]; } else { $pagename = $pagename[0]; }
			if (strtolower($pagename) == strtolower($getPage)) {
				$parts = scandir($contentpath.$getSubject."/".$folder."/".$page, 1);
				$page_check = 1;
				break;
				}
			}
	...
	if (!isset($page_check)) {
		...
		if (isset($_GET['subfolder'])) {
			$dir = $contentpath.$getSubject."/".$folder."/".$subfolder."/".$page; // Declare this variable so the whole directory structure doesn't have to be repeated below
			}
		else {
			$dir = $contentpath.$getSubject."/".$folder."/".$page;
			}
    unset($parts);
...
elseif (isset($_GET['subject'])) { // There needs to be a subject set. If there is at least this, but nothing else, then pull up the index page
	...
	$dir = $contentpath.$getSubject."/INDEX";
  unset($parts);
