// Make the preview popup appear in place of the links menu when hovering over the widget
  ...
  function scaffoldDiary(d, s, data) {
    // Grabs a reference to the element with the ID of diaryCalendar and clears it.
        ...
        var month = $('<h4/>', {class: 'month', text: d.format(' MMMM YYYY ')}).prepend(
          $('<a/>', {class: 'last', html: '<i class="fa fa-caret-left"></i>', href: "#"}).click(function(){generateDiary(d.clone().subtract(1, 'month'), s);})
        ).append(
          $('<a/>', {class: 'next', html: '<i class="fa fa-caret-right"></i>', href: "#"}).click(function(){generateDiary(d.clone().add(1, 'month'), s);})
        ).appendTo(diary);
  ...
  function generateDiary(d, s) {
    ...
    // Makes a sync call (wrapped by JQuery)
    $.ajax({
      url: jsonUrl,
      dataType:'JSON',
