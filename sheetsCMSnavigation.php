    <div class="row">
      <div class="col-xs-12">
        <h1><i class="fa fa-cog"></i> DCGS content management system</h1>
      </div>
    </div>
    <div class="row">
    <ul class="nav nav-tabs" role="tablist">
      <?php
        ...
        $tabs .= '<li role="presentation"><a href="#content" aria-controls="content" role="tab" data-toggle="tab">Update content</a></li>';
        $tabs .= '<li role="presentation"><a href="#hits" aria-controls="hits" role="tab" data-toggle="tab">Hits</a></li>';
        $tabs .= '<li role="presentation"><a href="#learn" aria-controls="learn" role="tab" data-toggle="tab">Learn websites</a></li>';
      ...
      <div class="row buttonLine">
        ...
        <!--<div class="row">
          <div class="col-xs-12">
            <h3>Other options</h3>
            <div class="row buttonLine">
              <div class="col-xs-12 col-sm-5 col-md-4"><a class="btn btn-default btn-block" href="/sync?action=clean">Clean stored data</a></div>
          ...
          if (!empty($sheet['sheetid'])) {
            $mainData['data']['sheets'][$sheet['sheetid']]['section'] = $sectionName;
              if (isset($mainData['data']['sheets'][$sheet['sheetid']]['lastupdate'])) {
                ...
                $button1 = '<a class="btn btn-default" href="/sync?sheet='.$sheet['sheetid'].'">Update content';
              } else {
                ...
                $button1 = '<a class="btn btn-success" href="/sync?sheet='.$sheet['sheetid'].'">Create content';
              }
    ...
    // Learn website controls
    echo '<div role="tabpanel" class="tab-pane fade';
      if ($tab == 'learn') {
        echo ' in active';
      }
    echo '" id="learn">';
    foreach ($learnSites as $learnSite => $siteID) {
      echo '<h3>'.ucwords($learnSite).'</h3>';
      echo '<div class="row">';
        echo '<div class="col-xs-12">';
          echo '<p><a class="btn btn-default" href="/sync?learn='.$learnSite.'">Update main data</a></p>';
        echo '</div>';
      echo '</div>';
      if (isset($learnData['data'][$learnSite])) {
        echo '<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">';
        foreach ($learnData['data'][$learnSite]['sections'] as $sectionName) {
          echo '<div class="panel panel-default content"><div class="panel-heading" role="tab" id="heading-'.clean($sectionName).'">';
            echo '<h4 class="panel-title">';
              echo '<a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse-'.clean($sectionName).'" aria-expanded="false" aria-controls="collapse-'.clean($sectionName).'">'.$sectionName.'</a>';
            echo '</h4>';
          ...
          echo '<div id="collapse-'.clean($sectionName).'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading-'.clean($sectionName).'">';
            echo '<div class="panel-body">';
            foreach ($learnData['data'][$learnSite]['sheets'] as $sheetID => $sheet) {
              if ($sheet['section'] == $sectionName) {
                if (isset($sheet['lastupdate'])) {
                      $name = $sheet['sheetname'];
                      $button1 = '<a class="btn btn-default" href="/sync?sheet='.$sheetID.'&learn='.$learnSite.'">Update content';
                    } else {
                      if (!empty($sheet['sheetname'])) {
                        $name = $sheet['sheetname'];
                      } else {
                      ...
                      $name = '<strong class="text-success">(New)</strong> '.$name;
                      $button1 = '<a class="btn btn-success" href="/sync?sheet='.$sheetID.'&learn='.$learnSite.'">Create content';
                    }
                  echo '<div class="row options"><div class="col-xs-12 col-xs-12 col-sm-3"><p>'.$name.':</p></div>';
                  echo '<div class="col-xs-12 col-sm-9 btn-group" role="group" aria-label="...">';
                    echo $button1.'</a>';
                    echo '<a class="btn btn-default" href="https://docs.google.com/spreadsheets/d/'.$sheetID.'" target="'.mt_rand().'">Edit spreadsheet</a>';
                    if (isset($sheet['lastupdate'])) {
                      echo '<a class="btn btn-default" href="http://'.$_SERVER['SERVER_NAME'].'/learn/'.$learnSite.'/'.clean($sectionName).'/'.clean($name).'" target="'.mt_rand().'">Visit content</a>';
                    }
                  echo '</div></div>';
              }
            }
    ...
    ?>
      ...
      <div role="tabpanel" class="tab-pane fade" id="viewdata"><?php view($mainData); ?></div>
    </div>
  </div>